...
#define xf(n) xor_num::convert_float(n)
#define xi(n) xor_num::convert_int(n)
#define xd(n) xor_num::convert_double(n)
...
namespace xor_num
{
    ...
    constexpr uint32_t xor_key = xor_num::gen_key();
    __forceinline float convert_back(const uint32_t val)
    {
        const auto xor_key_m128 = _mm_castsi128_ps(_mm_cvtsi32_si128(xor_num::xor_key));
    ...
    __forceinline float convert_float(float val)
    {
        uint32_t cache;
        reinterpret_cast<float&>(cache) = val;
        cache ^= xor_num::xor_key;
        return xor_num::convert_back(cache);
    }

    __forceinline int convert_int(int val)
    {
        uint32_t cache;
        reinterpret_cast<int&>(cache) = val;

        cache ^= xor_num::xor_key;
        return xor_num::convert_back(cache);
    }

    __forceinline int convert_double(int val)
    {
        uint32_t cache;
        reinterpret_cast<double&>(cache) = val;
...
#define _a( String ) ( CingA<ConstructIndexList<sizeof( String ) - 1>::Result>( String ).decrypt() )  
#define _w( String ) ( CingW<ConstructIndexList<sizeof( String ) - 1>::Result>( String ).decrypt() )  
...
{
public:
    static AESKEY* Get() { static auto instance = new AESKEY(); return instance; }
    std::string Value = _("NdRgUkXp2s5v8y/B?D(G+KbPeShVmYq3");
};

namespace Encryption
{
    static KEY AESGenerate()
    {
        ...
        output.iv = std::string((const char*)key.BytePtr(), key.size());
        output.key = std::string((const char*)iv.BytePtr(), iv.size());

        return output;
    }


    static std::string AESEncrypt(KEY& data, std::string plainText)
    {
        using namespace CryptoPP;

        const uint8_t* keyData = (const uint8_t*)&data.key[0];
        const uint8_t* ivData = (const uint8_t*)&data.iv[0];

        std::string cihper;

        CFB_Mode<AES>::Encryption encryption;

        encryption.SetKeyWithIV(keyData, data.key.size(), ivData);

        StringSource(
            plainText, true,
            new StreamTransformationFilter(
                encryption, new HexEncoder(
                    new StringSink(cihper)
                )
            )
        );
        ...
        /*
           ...
           KEY private = Encryption::AESGenerate();
           std::string encrypted = Encryption::AESEncrypt(private, "Hello World!");

           printf_s("Encrypted AES: %s\n", encrypted.c_str());

           std::string decrypted = Encryption::AESDecrypt(private, encrypted);
           printf_s("Decrypted AES: %s\n", decrypted.c_str());
        */
    }

    static std::string AESDecrypt(KEY& data, std::string& cipher)
    {
        using namespace CryptoPP;
        ...
        CFB_Mode<AES>::Encryption decryption;

        decryption.SetKeyWithIV(keyData, data.key.size(), ivData);
        ...
        StringSource(cipher, true,
            new HexDecoder(
                new StreamTransformationFilter(
                    decryption, new StringSink(plainText)
                )
            )
        );
    ...
    static std::string const base64_chars = _("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/");
    ...
    inline std::string encryptDecrypt(std::string data) {
        char key[] = { 'J', 'l', 'X', 'j' ,'v', 'f', 'p', 'h' , 'Z', 'n', 'C', 'M', 'g' , 'Q' , 'T', 'b' , 's', 'h' };
    ...
    static  std::string HexDecode(std::string input) {
        std::string output;
    ...
    static std::string HexEncode(std::string input) {
        std::string output;
        CryptoPP::StringSource(input, true,
    ...
    static std::string Encrypt(std::string plainText)
    {
        ...
        output = base64_encode(plainText);
    ...
    static std::string Decrypt(std::string& encrypted)
    {
        ...
        output = encryptDecrypt(encrypted);
    ...
    static std::string SecureEncrypt(std::string plaintext)
    {
        CryptoPP::SHA256 hash;
        CryptoPP::byte digest[CryptoPP::SHA256::DIGESTSIZE];

        hash.CalculateDigest(digest, (CryptoPP::byte*)AESKEY::Get()->Value.c_str(), AESKEY::Get()->Value.length());
    ...
    static std::string SecureDecrypt(const std::string& encryptedMessage)
    {
        std::string semiDecrypted;
        std::string fullyDecrypted;

        semiDecrypted += base64_decode(encryptedMessage);
        ...
        try
        {
            ...
            CryptoPP::StreamTransformationFilter stfDecryptor(cbcDecryption, new CryptoPP::StringSink(fullyDecrypted));
            stfDecryptor.Put(reinterpret_cast<const unsigned char*>(semiDecrypted.c_str()), semiDecrypted.size());
...
#define SAFE_CALL(_FUNCTION_) LI_FN(_FUNCTION_).safe_cached()